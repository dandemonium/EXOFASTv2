...
;   modelflux -=1d-6*reflect*sin(trueanom+omega0)*planetvisible*(sep^(-2.))
;     modelflux -= 1d-6*reflect*cos(2d0*!dpi*(transitbjd-tc)/period)*planetvisible*(sep^(-2d0))
;`   endelse
endif
...
if ellipsoidal ne 0d0 then begin
  ...
  if e eq 0 then begin
    ...
    ;  meananom += 2d0*!dpi*(transitbjd-tc)/period
     ; modelflux += 1d-6*ellipsoidal*(1d0-3d0*(sin(inc)^(2d0))*(sin(1d0*(trueanom)-omega0))^(2d0))/(sep^(3d0))
     modelflux += 1d-6*ellipsoidal*cos(2d0*ellphase)*sep^(-3d0); + e0*cos(omega0));*(sep^(-3.))
;;;;	modelflux -= 1d-6*ellipsoidal*sin(omega0 + 2d0*trueanom)*(sep^(-3d0))
	;modelflux += 1d-6*ellipsoidal*cos(4d0*!dpi*(transitbjd-tc)/period)*sep^(-3.)
  endelse
endif
...
;	modelflux-=1d-6*beam*sin(trueanom)*(1./SQRT(sep))
;	modelflux += 1d-6*beam*sin(2d0*!dpi*(transitbjd-tc0)/period)/SQRT(sep)
	modelflux += 1d-6*beam*(cos(trueanom+omega0) + e0*cos(omega0))
;      modelflux+=1d-6*beam*(cos(omega0)*((1.-(9.*e0^2d0/8.))*cos(2d0*meananom) + (e0 - 4d0*e0^(3d0)/3d0)*cos(2d0*meananom) + (9d0*e0^(2d0)/8d0)*cos(3d0*meananom) $
    ;              + (4d0*e0^(3d0)/3d0)*cos(4d0*meananom)) - sin(omega0)*((1d0-7d0*e0^(2d0)/8d0)*sin(meananom) + (e0 - 7d0*e^(3d0)/6d0)*sin(2d0*meananom) $
     ;               + 9d0*e0^(2d0)*sin(3d0*meananom)/8d0 + 4d0*e0^(3d0)*sin(4d0*meananom))) ;Murray & Dermott (1999)
   endelse
endif
